...
class DebouncerManager:
    """Manages multiple debouncers keyed by entity ID and property."""

    def __init__(self, delay: float = DEFAULT_DEBOUNCE_DELAY) -> None:
        """Initialize the manager.

        Args:
            delay: Default delay for all debouncers.
    ...
    def get_debouncer(self, key: str) -> Debouncer:
        """Get or create a debouncer for the given key.

        Args:
            key: Unique key (e.g., entity_id + property name).
        ...
        if key not in self._debouncers:
            self._debouncers[key] = Debouncer(self._delay)

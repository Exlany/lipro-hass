...
CapabilityResultGetter = Callable[
    ..., Awaitable[tuple[bool, CapabilityPayload | None, LiproApiError | None]]
]
...
async def async_handle_get_city(
    ...
    *,
    ...
    (
        ...
        result,
        last_err,
    ) = await async_get_first_authenticated_coordinator_capability_result(
...
async def async_handle_query_user_cloud(
    ...
    *,
    ...
    (
        ...
        collector=lambda coordinator: (
            coordinator.protocol_service.async_query_user_cloud()
        ),
    )
    if has_result and result is not None:
        return {"result": result}

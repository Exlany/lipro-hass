...
def bind_protocol_rest_port_family(rest_facade: object) -> ProtocolRestPortFamily:
    """Bind the canonical REST child façade into concern-local child-facing ports."""
    return ProtocolRestPortFamily(
        auth=cast(_RestAuthPort, rest_facade),
        inventory=cast(_RestInventoryPort, rest_facade),
        status=cast(_RestStatusPort, rest_facade),
        command=cast(_RestCommandPort, rest_facade),
        misc=cast(_RestMiscPort, rest_facade),
        schedule=cast(_RestSchedulePort, rest_facade),
        diagnostics=cast(_RestDiagnosticsPort, rest_facade),
    )

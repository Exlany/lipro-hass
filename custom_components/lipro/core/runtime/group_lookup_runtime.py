...
def compute_group_lookup_mapping_decision(
    *,
    ...
    gateway_register_id = current_gateway_id if isinstance(current_gateway_id, str) else None
    gateway_extra_data_value = gateway_register_id.strip() if gateway_register_id and gateway_register_id.strip() else None

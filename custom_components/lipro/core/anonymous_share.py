...
class SharedDevice:
    ...
    properties: dict[str, str]  # Sanitized property values
...
class AnonymousShareManager:
    ...
    def _add_error(
        ...
        for existing in self._errors:
            if existing.error_type == error_type and existing.message[:80] == msg_prefix:
                existing.count += 1
    ...
    def _sanitize_properties(self, properties: dict[str, Any]) -> dict[str, str]:

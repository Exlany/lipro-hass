...
_SENSITIVE_PATTERNS = (
    ...
    (re.compile(r'"password_hash"\s*:\s*"[^"]*"'), '"password_hash": "***"'),
    (re.compile(r'"passwordHash"\s*:\s*"[^"]*"'), '"passwordHash": "***"'),
    (
        re.compile(r'"phone"\s*:\s*"(?P<prefix>\+?)(?P<digits>\d{6,20})"'),
        _mask_phone_field,
    ),
    (re.compile(r'"install_token"\s*:\s*"[^"]*"'), '"install_token": "***"'),
    (re.compile(r'"installToken"\s*:\s*"[^"]*"'), '"installToken": "***"'),

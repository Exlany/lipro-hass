...
_INVALID_JSON_BODY_READ_MAX_BYTES = 8192
...
class LiproClient:
    ...
    def __init__(
        ...
        self._command_pacing_target_locks: dict[str, asyncio.Lock] = {}
    ...
    def _enforce_command_pacing_cache_limit(self) -> None:
        ...
        while len(tracked_targets) > _COMMAND_PACING_CACHE_MAX_SIZE:
            ...
            lock = self._command_pacing_target_locks.get(oldest_target)
            if lock is not None and not lock.locked():
                self._command_pacing_target_locks.pop(oldest_target, None)
    ...
    async def _throttle_change_state(self, target_id: str, command: str) -> None:
        ...
        async with self._command_pacing_lock:
            target_lock = self._command_pacing_target_locks.get(normalized_target)
            if target_lock is None:
                target_lock = asyncio.Lock()
                self._command_pacing_target_locks[normalized_target] = target_lock

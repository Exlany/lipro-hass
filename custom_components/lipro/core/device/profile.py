...
def resolve_device_type_hex(
    *,
    ...
    if physical_model and (
        resolved := PHYSICAL_MODEL_TO_DEVICE_TYPE.get(physical_model)
    ):
        return resolved
    if iot_name:
        normalized = IOT_NAME_TO_PHYSICAL_MODEL.get(
            iot_name
        ) or IOT_NAME_TO_PHYSICAL_MODEL.get(iot_name.lower())

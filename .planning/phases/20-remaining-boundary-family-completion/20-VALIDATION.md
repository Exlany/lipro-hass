...
| **Quick run command** | `uv run pytest -q tests/core/api/test_protocol_contract_matrix.py tests/core/api/test_schedule_codec.py tests/core/mqtt/test_mqtt_payload.py tests/core/mqtt/test_message_processor.py` |
| **Wave gate commands** | `uv run python scripts/check_architecture_policy.py --check` + `uv run python scripts/check_file_matrix.py --check` + `uv run pytest -q tests/meta/test_dependency_guards.py tests/meta/test_public_surface_guards.py tests/meta/test_governance_guards.py tests/meta/test_protocol_replay_assets.py tests/meta/test_version_sync.py` |
...
- **After wave 3 / before verification:** run wave gate commands, `uv run ruff check .`, `uv run mypy`, and the full suite command.
...
| 20-01-02 | 20-01 | 1 | SIM-03 | schedule-json boundary regression | `uv run pytest -q tests/core/api/test_schedule_codec.py tests/core/api/test_api_schedule_service.py tests/services/test_services_schedule.py` | ✅ | ⬜ pending |
...
| 20-02-01 | 20-02 | 2 | SIM-03 | MQTT topic boundary regression | `uv run pytest -q tests/core/mqtt/test_mqtt.py tests/core/mqtt/test_topic_builder.py` | ✅ | ⬜ pending |
| 20-02-02 | 20-02 | 2 | SIM-03 | MQTT message-envelope regression | `uv run pytest -q tests/core/mqtt/test_mqtt_payload.py tests/core/mqtt/test_message_processor.py tests/core/mqtt/test_protocol_replay_mqtt.py` | ✅ | ⬜ pending |
| 20-02-03 | 20-02 | 2 | SIM-03 | MQTT replay harness visibility | `uv run pytest -q tests/core/mqtt/test_protocol_replay_mqtt.py tests/integration/test_protocol_replay_harness.py tests/meta/test_protocol_replay_assets.py` | ✅ | ⬜ pending |
...
| `mqtt.topic.v1` / `mqtt.message-envelope.v1` do not turn message processor or transport utils into second authority roots | SIM-03 | topic/envelope formalization can drift into transport ownership ambiguity | Read updated docs/tests together and confirm message processor only consumes formal decoder truth |

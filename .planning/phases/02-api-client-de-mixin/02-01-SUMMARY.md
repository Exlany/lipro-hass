...
- 强化 `.planning/phases/02-api-client-de-mixin/02-ARCHITECTURE.md`，把 `LiproRestFacade`、`RestSessionState`、`RestTransportExecutor`、`RequestPolicy`、`RestAuthRecoveryCoordinator`、endpoint collaborators、payload normalizers、compat shell 的职责边界写成可执行裁决。
...
- `RestSessionState` 成为唯一 session/auth state owner；transport 只读状态，auth recovery 才能改 token / refresh state。
- `RestTransportExecutor`、`RequestPolicy`、`RestAuthRecoveryCoordinator`、endpoint collaborators、payload normalizers 被拆成显式协作者，禁止继续借由 mixin inheritance 隐式耦合。

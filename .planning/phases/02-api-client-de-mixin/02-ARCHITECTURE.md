...
| session / auth state | `ClientSessionState` | `AuthRecoveryCoordinator`, `TransportExecutor` | façade duplicates, endpoint helpers |
| transport / retry / pacing | `TransportExecutor` + `RequestPolicy` | `TransportCore`, `TransportRetry`, `TransportSigning` | ad-hoc helpers in `client.py`, mixin chain |
| auth recovery / replay | `AuthRecoveryCoordinator` | `AuthApiService`, token persistence callback, `TransportExecutor` | endpoint methods, compat wrappers |
...
|-----------------|--------------|----------|------------------------|
| `aiohttp.ClientSession` | `ClientSessionState` | `TransportExecutor` | endpoint collaborators、compat shell |
| `access_token` / `refresh_token` / `user_id` / `biz_id` | `ClientSessionState`（由 `AuthRecoveryCoordinator` 写入） | façade、transport、endpoint collaborators | `TransportExecutor`、normalizers |
| refresh single-flight lock | `ClientSessionState` + `AuthRecoveryCoordinator` | façade（只协调） | endpoint collaborators、compat shell |
| retry / backoff / pacing state | `RequestPolicy` | `TransportExecutor` | façade、endpoint collaborators、legacy mixins |
| raw vendor payload | `TransportExecutor` / endpoint collaborators（短暂存在） | payload normalizers | runtime/domain/entity |
...
`ClientSessionState` 是 auth/session state 的唯一事实源；任一 endpoint/helper 都不得自行缓存 token、refresh lock 或会话句柄。
...
认证恢复只能经 `AuthRecoveryCoordinator` 触发；refresh 后 replay 必须走同一 transport executor，并且只允许显式、安全、有限次重放。
...
4. `TransportExecutor` 通过 `ClientSessionState` 完成 encoding、signing、HTTP execution、response safety 与 telemetry。
5. 如遇认证失败，由 `AuthRecoveryCoordinator` 负责 refresh / replay；transport 本身不直接改 token。

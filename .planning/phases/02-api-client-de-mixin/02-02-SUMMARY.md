...
provides:
  ...
  - 显式 `RestSessionState` / `RestTransportExecutor` / `RestAuthRecoveryCoordinator` / `RequestPolicy` 协作主链
...
- 在 `custom_components/lipro/core/api/session_state.py` 中新增 `RestSessionState`，把 session / token / refresh lock / user/biz state 收口到单一状态根。
- 在 `custom_components/lipro/core/api/transport_executor.py` 中引入 `RestTransportExecutor`，把签名、会话获取、HTTP 执行、mapping-level retry 收敛到显式 transport collaborator。
- 在 `custom_components/lipro/core/api/auth_recovery.py` 中引入 `RestAuthRecoveryCoordinator`，把 auth 错误分类、refresh、replay、API 错误记录从继承链中剥离为独立协作者。
- 在 `custom_components/lipro/core/api/request_policy.py` 中引入 `RequestPolicy`，让 pacing / busy-retry / rate-limit policy 成为显式状态与策略对象。
- 保留 `client_pacing.py` / `transport_executor.py` / `auth_recovery.py` 中的薄兼容壳，使大量 legacy tests 与 patch points 在 `02-02` 不被一次性打碎。
- 更新 `tests/core/api/test_api_transport_executor.py`、`tests/core/api/test_helper_modules.py`、`tests/core/api/test_protocol_contract_matrix.py`，让测试既覆盖新 formal root/collaborator，又持续锁住 Phase 1 contract truth。

## 关键裁决

- **正式根裁决**：Phase 2 的正式 REST root 现在是 `LiproRestFacade`，不是 `LiproClient`。
- **状态所有权裁决**：session/auth/token/refresh single-flight 都归 `RestSessionState`；transport 不再拥有状态事实源。
- **恢复链裁决**：`RestAuthRecoveryCoordinator` 负责认证错误分类、refresh 与 replay 决策；transport 只做执行，不直接改 auth state。

...
- 回写 `FILE_MATRIX.md`，纠正 `client_base.py` / `client_auth_recovery.py` / `client_transport.py` / `endpoints/__init__.py` 等文件的真实定位：保留正式组件、仅把残余 compat spine 视为删除对象。
...
- **文件级裁决**：`client_base.py` / `client_transport.py` / `client_auth_recovery.py` / `endpoints/__init__.py` 这些文件本身不删除；真正待删除的是其中的 legacy mixin / compat spine。

...
| **Quick run command** | `uv run pytest -q tests/flows/test_config_flow.py tests/core/test_init.py tests/core/test_token_persistence.py tests/core/capability/test_registry.py tests/core/test_helpers.py tests/platforms/test_entity_behavior.py` |
| **Wave gate commands** | `uv run python scripts/check_architecture_policy.py --check` + `uv run python scripts/check_file_matrix.py --check` + `uv run pytest -q tests/meta/test_dependency_guards.py tests/meta/test_public_surface_guards.py tests/meta/test_governance_guards.py tests/meta/test_version_sync.py tests/meta/test_evidence_pack_authority.py` |
...
- **After wave 2:** Run the headless proof integration loop, device materialization proof, and replay/evidence bridge assertion suites.

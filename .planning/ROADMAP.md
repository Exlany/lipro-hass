...
- 🚧 **v1.30 Protocol Hotspot Convergence, Transport De-friendization & Snapshot Surface Slimming** - `Phase 107 -> 110` active on 2026-03-30; current route = `v1.30 active route / Phase 108 complete / latest archived baseline = v1.29`; latest archived evidence index = `.planning/reviews/V1_29_EVIDENCE_INDEX.md`; default next = `$gsd-discuss-phase 109`
...
**Milestone status:** `active / Phase 108 complete / continuation-ready (2026-03-30)`
...
**Current route mode:** `v1.30 active route / Phase 108 complete / latest archived baseline = v1.29`
**Default next command:** `$gsd-discuss-phase 109`
**Current follow-up target:** continue into anonymous-share manager inward decomposition via `Phase 109` while keeping `Phase 108` proof chain green.
...
**Status**: Complete (2026-03-30)
**Plans**: 3/3 complete
**Closeout assets**: `.planning/phases/108-mqtt-transport-runtime-de-friendization/{108-01-SUMMARY.md,108-02-SUMMARY.md,108-03-SUMMARY.md,108-VERIFICATION.md,108-VALIDATION.md}`
...
**Depends on:** latest archived baseline `v1.28`
**Requirements**: [ARC-26, TST-35, DOC-09, QLT-43]
**Status**: Complete (2026-03-28)
**Plans**: 3/3 complete
**Closeout assets**: `.planning/phases/103-root-adapter-thinning-test-topology-second-pass-and-terminology-contract-normalization/{103-01-SUMMARY.md,103-02-SUMMARY.md,103-03-SUMMARY.md,103-VERIFICATION.md,103-VALIDATION.md}`
...
**Goal:** 把 `tests/meta` / `scripts/check_*` 中重复的治理规则真源数据化，并把 planning / baseline / review / docs truth 收敛到同一条 `v1.29` active-route freeze / milestone-closeout handoff 语义。
**Depends on:** `Phase 104` complete
**Requirements**: [GOV-69, QLT-44]
**Status**: Complete (2026-03-28)
**Plans**: 3/3 complete
**Closeout assets**: `.planning/phases/105-governance-rule-datafication-and-milestone-freeze/{105-01-SUMMARY.md,105-02-SUMMARY.md,105-03-SUMMARY.md,105-SUMMARY.md,105-VERIFICATION.md,105-VALIDATION.md}`

## v1.28: Governance Portability, Verification Stratification & Open-Source Continuity Hardening

> `v1.28` 不重开新产品路线，而是把 latest archived baseline 切换过程中仍残留的治理脆弱点一次性收口：governance/meta smoke 对本机工具的耦合、verification matrix 当前真相与历史 closeout note 的混排、以及 docs-first / maintainer appendix continuity wording 的漂移都在本里程碑内完成归档硬化。

**Milestone Goal:** 沿 `v1.27` latest archived baseline 的唯一 north-star 起点，把 governance portability、verification stratification、portable docs-first wording 与 archive closeout bundle 压回同一条 archived-only baseline。
...
**Goal:** 把 governance/meta smoke 对本机 `node` / `gsd-tools` 的脆弱耦合收口为 capability-aware fast-path proof，把 `.planning/baseline/VERIFICATION_MATRIX.md` 当前 archived-only 真相与 historical phase closeout reference 明确分层，并把 docs-first / maintainer appendix / archive evidence wording 一次性前推到 `v1.28` archived-only closeout。 
**Depends on:** latest archived baseline `v1.27`
**Requirements**: [GOV-68, TST-34, OSS-13, QLT-42]
**Status**: Complete (2026-03-28)
**Plans**: 3/3 complete
**Closeout assets**: `.planning/phases/102-governance-portability-verification-stratification-and-open-source-continuity-hardening/{102-01-SUMMARY.md,102-02-SUMMARY.md,102-03-SUMMARY.md,102-VERIFICATION.md,102-VALIDATION.md}`

## v1.27: Final Carry-Forward Eradication & Route Reactivation

> `v1.27` 不是重新发明路线，而是在 `Phase 98 -> 100` 已完成 route reactivation 与 predecessor support freeze 之后，把 terminal audit 仍点名的 anonymous-share / REST-boundary hotspots 做最后一轮 inward decomposition，并最终完成 milestone audit、archive promotion 与 archived-only closeout 切换。

**Milestone Goal:** 沿 `v1.26` latest archived baseline 的唯一 north-star 起点，把诚实 carry-forward closure、runtime hotspot predecessor freeze、anonymous-share/REST-boundary hotspot decomposition、focused guards、phase assets 与最终 proof chain 压回同一条 archived-only baseline。
**Milestone status:** `archived / evidence-ready (2026-03-28)`
...
**Requirements**: [HOT-42, GOV-66, TST-32, QLT-40]
**Status**: Complete (2026-03-28)
**Plans**: 3/3 complete
**Closeout assets**: `.planning/phases/100-mqtt-runtime-and-schedule-service-support-extraction-freeze/{100-01-SUMMARY.md,100-02-SUMMARY.md,100-03-SUMMARY.md,100-VERIFICATION.md,100-VALIDATION.md}`

### Phase 101: Anonymous-share manager and REST decoder hotspot decomposition freeze
**Goal:** 把 `custom_components/lipro/core/anonymous_share/manager.py` 与 `custom_components/lipro/core/protocol/boundary/rest_decoder.py` / `rest_decoder_support.py` 这两处 remaining formal-home / boundary hotspots 收口为更诚实的 formal homes / inward collaborators，并把 current-route docs / ledgers / focused guards / GSD parser truth 一次性前推到 `Phase 101 complete`。
**Depends on:** `Phase 100` complete
**Requirements**: [HOT-43, GOV-67, TST-33, QLT-41]
**Status**: Complete (2026-03-28)
**Plans**: 3/3 complete
**Closeout assets**: `.planning/phases/101-anonymous-share-manager-and-rest-decoder-hotspot-decomposition-freeze/{101-01-SUMMARY.md,101-02-SUMMARY.md,101-03-SUMMARY.md,101-VERIFICATION.md,101-VALIDATION.md}`

## v1.26: Terminal Architecture Audit Follow-through, Typed Mapping Retirement & Hotspot Decomposition

> `v1.26` 承接 `v1.25` archived baseline，但没有把“已经基本正确”当作停止精修的理由：它把 broad typed seam、schedule/runtime hotspot、shared sanitizer burn-down 与 governance freeze 串成一条连续路线，并在 closeout 时把归档真相切回 `no active milestone route / latest archived baseline = v1.26`。

**Milestone Goal:** 把终极审计识别出的高收益 residual 收敛到单一 north-star 主链：先收窄 typed payload / domain bag / diagnostics broad seam，再 inward split schedule/runtime/sanitizer hotspots，最后用 governance/tests/docs freeze 把 archived baseline 与下一跳一起锁定。
**Milestone status:** `archived / evidence-ready (2026-03-28)`
...
**Milestone Goal:** 把“代码已经很强但仍有结构性边角”的问题集中收口：实体/平台不再直摸 runtime internals，runtime 装配归回单一 wiring root，governance 工具链脱离 `tests.helpers` / ad-hoc `sys.path`，对外入口文档/metadata 讲同一条公开协作故事。
...
Plans:
- [x] 89-01: entity-facing runtime verbs and OTA seam closure (completed 2026-03-27)
- [x] 89-02: runtime bootstrap single-wiring convergence (completed 2026-03-27)
- [x] 89-03: tooling helper ownership decoupling across architecture/file-matrix governance CLI roots (completed 2026-03-27)
- [x] 89-04: docs-first public entry and governance-route truth sync (completed 2026-03-27)

## v1.23: Repository-Wide Terminal Code Audit, Residual Eradication & Closeout Truth Freeze

> `v1.23` 已于 `2026-03-27` 完成 milestone audit、evidence-index closeout 与 archive promotion：它承接 `v1.22` 的 archived evidence，完成全域 terminal audit、production residual eradication、assurance hotspot no-regrowth freeze 与 governance sync / quality proof / milestone freeze，并把治理状态切换为 `no active milestone route / latest archived baseline = v1.23`。
...
**Closeout assets**: `.planning/phases/86-production-residual-eradication-and-boundary-re-tightening/{86-01-SUMMARY.md,86-02-SUMMARY.md,86-03-SUMMARY.md,86-04-SUMMARY.md}`
...
**Executed summaries**: `.planning/phases/87-assurance-hotspot-decomposition-and-no-regrowth-guards/{87-01-SUMMARY.md,87-02-SUMMARY.md,87-03-SUMMARY.md,87-04-SUMMARY.md}`
...
> `v1.21` 已于 `2026-03-26` 完成 archive promotion：它承接 `v1.20` 的 archived evidence，完成 machine-readable route activation、bootstrap smoke topicization、route-handoff quality gate formalization、registry hotspot decomposition、release-contract topicization 与 typing closure，并把治理状态切到 `no active milestone route / latest archived baseline = v1.21`。
...
**Archive assets:** `.planning/v1.21-MILESTONE-AUDIT.md`, `.planning/reviews/V1_21_EVIDENCE_INDEX.md`, `.planning/milestones/v1.21-ROADMAP.md`, `.planning/milestones/v1.21-REQUIREMENTS.md`
...
**Status**: Complete (2026-03-25)
**Plans:** 4/4 complete across 3 waves
...
**Status**: Complete (2026-03-25)
**Plans:** 4/4 complete across 4 waves
...
**Goal:** 让 private-access honest story、promoted evidence allowlist、thin-adapter typed honesty 与 milestone archive truth 讲同一条收官叙事，并把 `v1.20` 的 historical archive-transition route truth 明确写为 `no active milestone route / latest archived baseline = v1.20`。
**Depends on:** Phase 74
**Requirements**: [GOV-56, ARC-19, TYP-21, TST-22, QLT-30]
**Status**: Complete (2026-03-25)
**Plans:** 4/4 complete across 2 waves
...
> `v1.19` 已于 `2026-03-25` 完成 milestone audit、evidence-index closeout 与 archive promotion：它承接 `v1.18` 的 archived baseline，完成 OTA diagnostics / firmware-install、anonymous-share submit、request pacing / command-runtime 的 final hotspot decomposition，并完成 archive-promotion transition；后续在 `v1.21` archive promotion 中，`v1.20` 退为 previous archived baseline，`v1.19` 退为 historical archived baseline。

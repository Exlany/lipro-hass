...
- [x] **ARC-41**: `custom_components/lipro/core/coordinator/runtime/command_runtime.py` 与 `custom_components/lipro/entities/firmware_update.py` 必须继续 inward split，把多主题逻辑压回更窄 helper/policy seams，同时保持单一正式主链与 typed contract。
- [x] **HOT-60**: 新一轮 hotspot burn-down 必须优先处理 currently sanctioned formal homes 中仍然过重的 multi-topic files，至少冻结 `command_runtime.py` / `firmware_update.py` 的 no-regrowth 方向与下一波拆分边界。
...
**Milestone Goal:** 基于 `v1.35` latest archived baseline，`v1.36` 已把终审 carry-forward residual 全部完成 closeout：diagnostics helper shell thinning / duplicate mechanics 已收口，`runtime_access` de-reflection / typed telemetry seam 已显式化，开源 readiness / benchmark / coverage / continuity gap 也已收束为 archived governance contract。
**Milestone status:** `archived / evidence-ready (2026-04-01)`
**Current route mode:** `no active milestone route / latest archived baseline = v1.36`
...
**Current phase handoff:** `Milestone closeout complete；Phase 126 -> 128 requirement coverage 已冻结为 archived evidence frozen，后续增量工作应由 `$gsd-new-milestone` 重新立项。`
...
- [x] **GOV-85**: `PROJECT / ROADMAP / REQUIREMENTS / STATE / MILESTONES / GOVERNANCE_REGISTRY` 必须共同承认 `no active milestone route / latest archived baseline = v1.36`，并把 `Phase 126 -> 128` 的 archived closeout truth 讲成一条诚实 current story。
...
**Milestone Goal:** 基于 `v1.34` latest archived baseline，把全仓 master audit 的 terminal residual 在 closeout 前彻底压回单一 active route：除已完成的 `service_router` / auth-seed / schedule contract 收口外，还要继续拆解 `runtime_types.py` 的 cross-plane contract hub、进一步压薄 `config_flow.py` / `entry_auth.py` adapter seam，并把 current-route truth 变成 machine-readable governance contract。
**Milestone status:** `archived / evidence-ready (2026-04-01)`
**Current route mode:** `no active milestone route / latest archived baseline = v1.35`
...
**Default next command:** `$gsd-new-milestone`
**Archived evidence index:** `.planning/reviews/V1_35_EVIDENCE_INDEX.md`
**Current phase handoff:** `Milestone closeout complete; Phase 122 -> 125 的 summary / verification / audit / evidence index 已提升为 archived milestone truth。`
...
### Terminal Residual Eradication & Governance Extraction
- [x] **ARC-37**: `custom_components/lipro/runtime_types.py` 不得继续充当跨平面 mega contract hub；runtime / protocol / service-facing protocol families 必须 inward 到更窄 formal homes，outward import truth 仍保持单一可发现。
- [x] **HOT-56**: `custom_components/lipro/config_flow.py` 与 `custom_components/lipro/entry_auth.py` 必须继续瘦身为 HA-root adapter / bootstrap home；纯 pass-through step orchestration、result shaping 与 transient auth helpers 必须沉到 `flow/` 或更窄 helper。
- [x] **GOV-84**: `PROJECT / ROADMAP / REQUIREMENTS / STATE / MILESTONES` 的 current-route truth 必须共同消费同一 machine-readable governance contract，而不是五份并行手写 prose/yaml selector。
- [x] **TST-47**: `tests/meta` 与相关 checker 必须优先读取 machine-readable governance contract / focused manifest，减少 giant prose-heavy wording guards；语义不变的文案调整不应再引发大面积 brittle failures。
- [x] **QLT-49**: `runtime_types.py`、`config_flow.py`、`entry_auth.py`、`tests/meta` 与相关 governance tooling 必须建立新的 no-regrowth / changed-surface budget，避免 residual hotspot 在 closeout 后重新扩张。
- [x] **DOC-14**: maintainer / developer docs 必须解释 final residual topology、machine-readable governance truth 的正式 home，以及为何 `Phase 125` 是 v1.35 closeout 前最后一轮 carry-forward。

## Traceability

| Requirement | Phase | Status |
|-------------|-------|--------|
| AUD-05 | Phase 122 | Complete |
| DOC-12 | Phase 122 | Complete |
...
**Milestone status:** `archived / evidence-ready (2026-04-01)`
**Current route mode:** `no active milestone route / latest archived baseline = v1.34`
...
**Default next command:** `$gsd-new-milestone`
**Current phase handoff:** `Milestone closeout complete; Phase 120 + 121 的 summary / verification / audit / evidence index 已提升为 archived milestone truth。`
...
**Milestone status:** `archived / evidence-ready (2026-04-01)`
**Current route mode:** `no active milestone route / latest archived baseline = v1.33`
...
**Current audit artifact:** `.planning/v1.31-MILESTONE-AUDIT.md`

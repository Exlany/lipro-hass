...
- ✅ **v1.30 Protocol Hotspot Convergence, Transport De-friendization & Snapshot Surface Slimming** - `Phase 107 -> 110` archived on 2026-03-30; current route = `no active milestone route / latest archived baseline = v1.30`; latest archived evidence index = `.planning/reviews/V1_30_EVIDENCE_INDEX.md`; default next = `$gsd-new-milestone`
...
> `v1.30` 沿 `v1.29` latest archived baseline 的唯一 north-star 主线，把 `Phase 106` evidence-only 审计点名的 remaining hotspots 全量收口并完成归档：`Phase 107` 已收口 REST/auth/status 家族 hotspots，`Phase 108` 已完成 MQTT transport/runtime de-friendization，`Phase 109` 已完成 anonymous-share manager inward decomposition，`Phase 110` 已完成 snapshot surface slimming 与 milestone closeout。

# Requirements Archive: Lipro-HASS v1.1 Protocol Fidelity & Operability

## Archive Note

本归档文件保留 `v1.1` requirement ledger 的最终快照；内容以当前活跃 requirements 真源为基础抽取 `v1.1` 范围，用于停止让后续里程碑复用同一份 requirements history。

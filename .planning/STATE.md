...
**Current mode:** `v1.30 active route / Phase 110 complete / latest archived baseline = v1.29`
...
- **Current follow-up target:** execute `$gsd-complete-milestone v1.30`，同时保持 `Phase 107/108/109` predecessor visibility 与 `v1.29` latest archived baseline truth 不漂移
...
1. `$gsd-complete-milestone v1.30` —— 执行 `v1.30` 里程碑正式封板与 archived selector 翻转。
2. `$gsd-next` —— 复核自动路由是否稳定落到 `$gsd-complete-milestone v1.30`。
3. `node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" state json` —— 复核 parser-stable `milestone = v1.30` 与 `status = active`。
4. `node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" init progress` —— 复核 `Phase 107..110` 全部 complete、`current_phase = null` 与 `has_work_in_progress = false`。
5. `node "$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" phase-plan-index 110` —— 复核 `Phase 110` 六个 plans 全部归档可见。
6. `uv run pytest -q tests/meta/test_governance_route_handoff_smoke.py tests/meta/governance_followup_route_current_milestones.py tests/meta/test_phase105_governance_freeze_guards.py tests/meta/test_phase107_rest_status_hotspot_guards.py tests/meta/test_phase108_mqtt_transport_de_friendization_guards.py tests/meta/test_phase109_anonymous_share_manager_inward_decomposition_guards.py tests/meta/test_phase110_runtime_snapshot_closeout_guards.py` —— 复核 current-route / predecessor / latest-archived / focused-guard 一致性。

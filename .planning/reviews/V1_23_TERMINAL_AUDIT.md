...
**Updated:** 2026-03-27  
**Route truth:** `v1.23 active route / Phase 85 complete / latest archived baseline = v1.22`  
...
2. **`Phase 86 route next` production carry-forward**：`share_client.py` 仍保留 silent compat alias / bool shim，`runtime_infra.py` 仍是 formal-home-correct 但职责过厚的 production hotspot。
...
| File | Severity | Verdict | Owner | Exit condition | Evidence |
|---|---|---|---|---|---|
| `custom_components/lipro/core/anonymous_share/share_client.py` | Medium | route next | `Phase 86` (`HOT-37`, `ARC-22`) | `_safe_read_json()` backward-compatible alias 与 bool-only `submit_share_payload()` shim 删除或 inward 收口；production/tests 只承认 outcome-native submit path | `custom_components/lipro/core/anonymous_share/share_client.py`, `.planning/reviews/KILL_LIST.md` |
| `custom_components/lipro/runtime_infra.py` | Medium | route next | `Phase 86` (`HOT-37`, `ARC-22`) | listener / reload / bootstrap / task bookkeeping concern 继续 inward split；formal home 保留，但不再由单文件吸附过厚控制面职责 | `custom_components/lipro/runtime_infra.py`, `.planning/reviews/RESIDUAL_LEDGER.md` |
...
|---|---|---|
| `Phase 86` | production residual eradication | `share_client.py` compat aliases / bool shim, `runtime_infra.py` hotspot |
...
- `share_client.py` 当前只登记 symbol-level delete gates；在 `Phase 86` 之前，它仍是正式 anonymous-share worker client home。

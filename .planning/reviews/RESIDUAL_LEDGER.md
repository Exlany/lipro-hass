...
- latest archived baseline 现已提升到 `v1.19`；当前治理状态为 `no active milestone route / latest archived baseline = v1.19`，且本 phase 不新增 ownerless carry-forward。
...
- milestone truth 已完成 archive promotion：当前治理状态为 `no active milestone route / latest archived baseline = v1.19`；`v1.18` 退为 previous archived baseline。

...
| `custom_components/lipro/core/anonymous_share/manager.py` | Protocol | Phase 90 / 113 / 118 | 重构 | formal anonymous-share aggregate manager home with scope-state support collaborators; Phase 118 continues bounded orchestration slimming after the Phase 113 no-growth freeze |
...
| `custom_components/lipro/core/anonymous_share/sanitize.py` | Protocol | Phase 92 | 保留 | structure-preserving anonymous-share sanitizer on shared redaction contract; structure-preserving anonymous-share sanitizer with container/scalar helper split |
...
| `custom_components/lipro/core/api/auth_recovery.py` | Protocol | Phase 2 / 35 | 重构 | REST auth-recovery refresh / replay collaborator home |
...
| `custom_components/lipro/core/api/status_fallback_support.py` | Assurance | Phase 94 / 95 / 96 / 97 / 98 / 99 / 100 / 101 / 102 / 113 / 118 | 保留 | status fallback local recursion/logging support collaborator; Phase 118 resumes inward split after the Phase 113 no-growth hotspot budget freeze |
...
| `custom_components/lipro/core/coordinator/runtime/device/snapshot_support.py` | Runtime | Phase 5 | 重构 | - |
...
| `custom_components/lipro/core/coordinator/runtime/mqtt_runtime_support.py` | Assurance | Phase 94 / 95 / 96 / 97 / 98 / 99 / 100 / 101 / 102 | 保留 | MQTT-runtime local transport/notification/background-task support collaborator |
...
| `custom_components/lipro/core/protocol/boundary/rest_decoder.py` | Protocol | Phase 101 / 113 / 118 | 保留 | REST boundary decoder family home; Phase 118 continues family/topic split after the Phase 113 no-growth hotspot budget freeze |
...
| `custom_components/lipro/core/telemetry/exporter.py` | Assurance | Phase 92 | 保留 | shared-policy telemetry exporter with pseudonymous alias + marker summary budget contract; shared-policy telemetry exporter with localized sanitize helper split |
...
| `custom_components/lipro/entities/firmware_update.py` | Domain | Phase 90 / 113 / 118 | 保留 | protected thin OTA projection shell after runtime-boundary tightening; Phase 118 revisits the remaining orchestration density under the same outward contract |
...
| `tests/core/api/test_api_diagnostics_service_cloud.py` | Protocol | Phase 2 | 保留 | - |
| `tests/core/api/test_api_diagnostics_service_history.py` | Protocol | Phase 2 | 保留 | - |
| `tests/core/api/test_api_diagnostics_service_ota.py` | Protocol | Phase 2 | 保留 | - |
...
| `tests/core/api/test_protocol_contract_boundary_decoders.py` | Protocol | Phase 2 | 保留 | - |
| `tests/core/api/test_protocol_contract_facade_runtime.py` | Protocol | Phase 2 | 保留 | - |
| `tests/core/api/test_protocol_contract_fixture_authority.py` | Protocol | Phase 2 | 保留 | - |
...
| `tests/core/coordinator/runtime/test_mqtt_runtime_init.py` | Runtime | Phase 5 / 6 | 保留 | - |
| `tests/core/coordinator/runtime/test_mqtt_runtime_messages.py` | Runtime | Phase 5 / 6 | 保留 | - |
| `tests/core/coordinator/runtime/test_mqtt_runtime_notifications.py` | Runtime | Phase 5 / 6 | 保留 | - |
...
| `tests/core/coordinator/runtime/test_snapshot_support.py` | Runtime | Phase 5 / 6 | 保留 | - |
...
| `tests/harness/__init__.py` | Assurance | Phase 8 | 保留 | - |
...
| `tests/integration/test_protocol_replay_harness.py` | Assurance | Phase 8 | 保留 | - |
...
| `tests/meta/governance_current_truth.py` | Assurance | Phase 6 / 77 / 79 / 105 / 118 | 保留 | governance-route contract + shared current/latest archive truth helper for the active selector family |
...
| `tests/meta/governance_followup_route_current_milestones.py` | Assurance | Phase 79 / 105 / 118 | 保留 | governance-route contract + current/latest archive pointer-drift guard for the active milestone story |
| `tests/meta/governance_followup_route_specs.py` | Assurance | Phase 105 | 保留 | shared follow-up route spec + planning-doc snapshot helper home |
...
| `tests/meta/test_governance_closeout_guards.py` | Assurance | Phase 27 / 44 / 49 / 77 / 88 / 105 | 保留 | closeout + promoted-asset manifest smoke anchor + milestone-freeze exit-contract bridge |
...
| `tests/meta/test_governance_route_handoff_smoke.py` | Assurance | Phase 79 / 105 / 118 | 保留 | route-handoff gsd fast-path smoke guard home |
...
| `tests/meta/test_phase112_formal_home_governance_guards.py` | Assurance | Phase 112 / 118 | 保留 | focused active-route / sanctioned-home / archive-pointer guard home for Phase 112 governance normalization and Phase 118 route wording |
...
| `tests/meta/test_phase97_governance_assurance_freeze_guards.py` | Assurance | Phase 94 / 95 / 96 / 97 / 98 / 99 / 100 / 101 / 102 | 保留 | focused closeout guard home for Phase 97 governance / assurance freeze |
| `tests/meta/test_phase98_route_reactivation_guards.py` | Assurance | Phase 94 / 95 / 96 / 97 / 98 / 99 / 100 / 101 / 102 | 保留 | focused predecessor guard home for Phase 98 reactivation / carry-forward closure |
...
| `tests/meta/test_version_sync.py` | Assurance | Phase 6 / 77 / 79 / 105 / 114 / 118 | 保留 | version/runtime metadata sync guard home + schema-limited projection truth + active-route docs alignment |

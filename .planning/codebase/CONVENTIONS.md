...
- **API collaborator 命名残留继续收口**：`LiproRestFacade` 的正式协作者已经显式化为 `RestRequestGateway` / `RestAuthRecoveryCoordinator` / `RestTransportExecutor` / `RestEndpointSurface`；剩余命名余味主要集中在 `client.py` 与 `client_pacing.py` 这类 stable import / pacing 入口的历史文件名。证据：`custom_components/lipro/core/api/client.py:1`, `custom_components/lipro/core/api/request_gateway.py:1`, `custom_components/lipro/core/api/client_pacing.py:1`。

...
1. **Clone the repository / 克隆仓库**
   ...
   This is the canonical GitHub repository for the integration.
   这是本集成当前的正式 GitHub 仓库地址。
...
Current repository note: root docs and `docs/` stay the canonical first hop for users and contributors, while GitHub Issues / Discussions / Security UI remain the follow-up intake surfaces.
当前仓库说明：根目录文档与 `docs/` 仍是用户和贡献者的正式 first hop，GitHub Issues / Discussions / Security UI 作为后续反馈入口。
...
- Shared docs-routing truth lives in `CONTRIBUTING.md`, `docs/README.md`, and GitHub templates; keep those surfaces aligned.
...
- `docs/MAINTAINER_RELEASE_RUNBOOK.md` 可保持 maintainer-only；公开入口可按需显式链接过去，但不得取代 public first hop。

- If you touch `README.md` / `README_zh.md` / `CONTRIBUTING.md` / `SUPPORT.md` / `SECURITY.md` / `.github/*` / release workflow, update `docs/README.md`, `docs/TROUBLESHOOTING.md`, `docs/MAINTAINER_RELEASE_RUNBOOK.md`, and any directly affected public navigation at the same time.

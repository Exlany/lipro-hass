# Lipro Home Assistant Integration - Architecture Archive Appendix

> **Role**: pull-only architecture appendix for historical phase notes and archive reachability.
> **Current selector reminder**: live route truth stays in `.governance/{PROJECT,ROADMAP,REQUIREMENTS,STATE,MILESTONES}.md`; this file does not act as current-route truth.
> **Latest archived evidence pointer**: `.governance/reviews/V1_33_EVIDENCE_INDEX.md`
> **Latest archived milestone audit**: `.governance/v1.33-MILESTONE-AUDIT.md`
...
- Phase 89 closeout treats `V1_23_TERMINAL_AUDIT.md` as historical input evidence only; archived-only route truth now lives in `.governance/{PROJECT,ROADMAP,REQUIREMENTS,STATE,MILESTONES}.md`, `.governance/v1.33-MILESTONE-AUDIT.md`, `.governance/reviews/V1_33_EVIDENCE_INDEX.md`, and focused governance guards.
...
- `.governance/reviews/PROMOTED_PHASE_ASSETS.md` 是 phase evidence allowlist 的唯一 home；developer-facing guidance 只索引它，不私自提升 execution traces。
...
- `Phase 97` 现在承担 `v1.26` latest archived closeout proof：developer guidance、governance route contract、focused assurance guards 与 archive pointer 共同证明 `v1.26` 已稳定退回 pull-only archived baseline。
...
- `Phase 102` note 记录的是当时的 archived-only selector truth：当 `v1.33` 作为 latest archived baseline 时，developer guidance 与 governance docs 曾共同承认 `no active milestone route / latest archived baseline = v1.33`；它是历史上下文，不构成当前 live route claim。
...
- `Phase 110` closeout bundle、`.governance/reviews/V1_30_EVIDENCE_INDEX.md` 与 `.governance/v1.30-MILESTONE-AUDIT.md` 已形成可审计证据链，当前 follow-up 固定为 `$gsd-new-milestone`。

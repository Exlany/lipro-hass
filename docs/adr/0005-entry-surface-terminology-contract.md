...
这类噪音不会立刻造成功能错误，却会持续放大 onboarding、code review、phase governance 与 residual 审计的理解成本。尤其是根入口 thin adapter、control service callback family 与 protocol-plane façade 在同一轮重构中同时出现时，若术语没有契约，文档与测试很容易再次讲成两套故事。
...
- review 与 phase governance 能更快判断“这是 formal home 还是 inward support”

...
- `__init__.py`: Integration setup, service schemas, platform forwarding.
...
- `device/`: `LiproDevice` model + refresh/index/group/outlet helpers.
...
- `coordinator.py`: The orchestration hub (`DataUpdateCoordinator`) that binds HA runtime to core logic.
...
- `core/coordinator.py` is the exception: it integrates HA runtime APIs with the core packages.
...
Internal helpers live in subpackages (`core/api/*`, `core/command/*`, `core/runtime/*`, ...).
...
| `custom_components.lipro.core.room_sync_runtime` | `custom_components.lipro.core.runtime.room_sync_runtime` |
| `custom_components.lipro.core.coordinator_runtime` | `custom_components.lipro.core.runtime.coordinator_runtime` |

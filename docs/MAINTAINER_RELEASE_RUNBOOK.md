...
> Current route note / 当前路线说明：maintainer continuity now follows the stable selector family `.planning/{PROJECT,ROADMAP,REQUIREMENTS,STATE,MILESTONES}.md`; today it resolves to `no active milestone route / latest archived baseline = v1.36`, current status = `archived / evidence-ready (2026-04-01)`, default next command = `$gsd-new-milestone`. Latest archived evidence remains pull-only: `.planning/reviews/V1_36_EVIDENCE_INDEX.md` + `.planning/v1.36-MILESTONE-AUDIT.md`.

...
> Current route note / 当前路线说明：maintainer continuity currently follows `no active milestone route / latest archived baseline = v1.32`; current handoff = `v1.32 archived / evidence-ready`, default next command = `$gsd-new-milestone`; live selector truth stays in `.planning/{PROJECT,ROADMAP,REQUIREMENTS,STATE}.md`, while `.planning/MILESTONES.md` remains archive chronology only.
...
- Canonical route-selector family: `.planning/{PROJECT,ROADMAP,REQUIREMENTS,STATE}.md` (`no active milestone route / latest archived baseline = v1.32`; current handoff = `v1.32 archived / evidence-ready`; default next = `$gsd-new-milestone`)

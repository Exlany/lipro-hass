...
> Current route note / 当前路线说明：maintainer continuity currently follows `no active milestone route / latest archived baseline = v1.31`; live selector truth stays in `.planning/{PROJECT,ROADMAP,REQUIREMENTS,STATE}.md`, while `.planning/MILESTONES.md` remains archive chronology only.
...
- Canonical active-route selector family: `.planning/{PROJECT,ROADMAP,REQUIREMENTS,STATE}.md` (`no active milestone route / latest archived baseline = v1.31`)

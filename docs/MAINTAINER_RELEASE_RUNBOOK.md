...
> Current route note / 当前路线说明：maintainer continuity now follows the stable selector family `.planning/{PROJECT,ROADMAP,REQUIREMENTS,STATE,MILESTONES}.md`; today it resolves to `v1.36 active milestone route / starting from latest archived baseline = v1.35`, current status = `active / phase 128 complete; closeout-ready (2026-04-01)`, default next command = `$gsd-complete-milestone v1.36`. Latest archived evidence remains pull-only: `.planning/reviews/V1_35_EVIDENCE_INDEX.md` + `.planning/v1.35-MILESTONE-AUDIT.md`.

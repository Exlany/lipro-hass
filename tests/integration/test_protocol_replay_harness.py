...
def test_protocol_replay_harness_runs_all_registered_manifests_in_order() -> None:
    ...
    assert all(
        result.canonical == EXPECTED_MQTT_CONFIG for result in mqtt_config_results
    )


def test_protocol_replay_harness_pulls_telemetry_assertions_from_exporter_truth() -> (
    None
):
    driver = ProtocolReplayDriver()
...
def test_protocol_replay_harness_builds_structured_run_summary() -> None:
    ...
    assert summary["scenarios"][0]["telemetry_alignment"]["device_ref"].startswith(
        "device_"
    )

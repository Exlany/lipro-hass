...
@pytest.mark.asyncio
async def test_async_ensure_authenticated_and_trigger_reauth_delegate_to_auth_service() -> (
    None
):
    coordinator = Mock()
    ...
    coordinator.auth_service.async_trigger_reauth.assert_awaited_once_with(
        RuntimeReauthReason.AUTH_ERROR
    )
...
@pytest.mark.asyncio
async def test_async_capture_coordinator_call_triggers_reauth_and_returns_auth_error() -> (
    None
):
    coordinator = Mock()
    coordinator.auth_service = Mock(
...
@pytest.mark.asyncio
async def test_async_execute_coordinator_call_triggers_reauth_for_expired_token() -> (
    None
):
    coordinator = Mock()
    coordinator.auth_service = Mock(
    ...
    coordinator.auth_service.async_trigger_reauth.assert_awaited_once_with(
        RuntimeReauthReason.AUTH_EXPIRED
    )

...
@pytest.mark.parametrize(
    "payload",
    [
        {"days": [], "times": [3600], "events": [1]},
        {"days": [1], "times": [], "events": [1]},
        {"days": [1], "times": [3600], "events": []},
    ],
)
def test_normalize_add_schedule_payload_rejects_empty_schedule_lists(
    payload: dict[str, list[int]],
) -> None:
    """add_schedule should reject empty list payloads before reaching the API."""
    with pytest.raises(vol.Invalid):
        normalize_add_schedule_payload(payload)
...
def test_normalize_delete_schedules_payload_rejects_empty_id_list() -> None:
    """delete_schedules should not accept a no-op empty id list."""
    with pytest.raises(vol.Invalid):
        normalize_delete_schedules_payload({"schedule_ids": []})

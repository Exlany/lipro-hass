...
@pytest.mark.asyncio
async def test_async_handle_submit_anonymous_share_failure_returns_typed_outcome() -> (
    None
):
    hass = cast(HomeAssistant, MagicMock())
...
@pytest.mark.asyncio
async def test_async_handle_submit_anonymous_share_preserves_auth_failure_metadata() -> (
    None
):
    hass = cast(HomeAssistant, MagicMock())

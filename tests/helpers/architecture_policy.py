...
_POLICY_RELATIVE_PATH = Path(".planning/baseline/ARCHITECTURE_POLICY.md")
...
def policy_root(start: Path | None = None) -> Path:
    ...
    return repo_root(start or Path(__file__))
...
def _build_rule(row: dict[str, str]) -> ArchitecturePolicyRule:
    return ArchitecturePolicyRule(
        ...
        allowed_or_required_signals=_split_multivalue(row["Allowed / Required Signals"]),
...
def load_structural_rules(root: Path | None = None) -> dict[str, ArchitecturePolicyRule]:

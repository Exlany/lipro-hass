...
class TestInitSetupEntryBehavior(_InitRuntimeBehaviorBase):
    ...
    async def test_async_setup_entry_wires_runtime_data_to_coordinator(
        self, hass
    ) -> None:
        ...
        with (
            patch(
                "custom_components.lipro.async_ensure_runtime_infra", new=AsyncMock()
            ),
            patch(
            ...
            patch(
                "custom_components.lipro.Coordinator", return_value=coordinator
            ) as ctor,

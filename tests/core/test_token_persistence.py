...
def test_persist_entry_tokens_clears_stale_biz_id(hass) -> None:
    """Cleared biz_id should remove any stale persisted value from the entry."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        data={
            CONF_PHONE_ID: "phone-id",
    ...
    auth_manager.get_auth_session.return_value = MagicMock(
    ...
    assert data[CONF_ACCESS_TOKEN] == "new_access"
    assert data[CONF_REFRESH_TOKEN] == "new_refresh"
    assert CONF_BIZ_ID not in data

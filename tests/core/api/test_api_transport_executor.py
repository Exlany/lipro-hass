...
@pytest.mark.asyncio
async def test_execute_mapping_request_with_rate_limit_uses_request_policy_home() -> (
    None
):
    ...
    with patch.object(
        ...
        (
            ...
            result,
            request_token,
        ) = await executor.execute_mapping_request_with_rate_limit(
            path="/v2/test",
            retry_count=0,
...
def test_to_device_type_hex_accepts_decimal_strings_and_rejects_invalid_format() -> (
    None
):
    executor = _build_executor()

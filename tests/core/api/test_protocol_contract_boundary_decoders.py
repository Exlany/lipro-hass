...
DEVICE_LIST_FIXTURE_NAMES = (
    "get_device_list.direct.json",
...
def test_rest_decoder_public_home_reexports_family_registry_and_utility_collaborators() -> (
    None
):
...
def test_protocol_root_contracts_normalize_device_status_rows() -> None:
    ...
    assert (
        client.contracts.normalize_device_status_rows(payload)
        == EXPECTED_DEVICE_STATUS_ROWS
    )
...
def test_rest_boundary_decoder_excludes_nested_and_metadata_fields_from_fallback_properties() -> (
    None
):
    payload = {
        "data": [

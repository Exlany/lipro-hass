...
class TestLiproClient429Handling:
    ...
    def test_parse_retry_after_http_date_without_timezone_assumes_utc(self):
        """HTTP date without tzinfo should be treated as UTC."""
        from datetime import UTC, datetime, timedelta
...
class TestLiproClientAdditionalBranchCoverage:
    ...
    def test_enforce_command_pacing_cache_limit_handles_empty_last_change_state_at_and_drops_idle_lock(
        ...
        client._change_state_min_interval[target] = 0.2
        client._change_state_busy_count[target] = 1
        client._command_pacing_target_locks[target] = asyncio.Lock()

        client._enforce_command_pacing_cache_limit()

        assert target not in client._change_state_min_interval
        assert target not in client._change_state_busy_count
        assert target not in client._command_pacing_target_locks

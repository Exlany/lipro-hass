...
async def test_query_with_fallback_direct_success_records_zero_depth() -> None:
    ...
    result = await query_with_fallback(
        ...
        item_name="device",
        iot_request=AsyncMock(
            return_value={"data": [{"deviceId": "a"}, {"deviceId": "b"}]}
        ),
        extract_data_list=_extract_rows,
        is_retriable_device_error=lambda _: True,
        lipro_api_error=_DummyApiError,
...
async def test_query_with_fallback_rejects_negative_small_subset_threshold(
    ...
    monkeypatch.setattr(
        status_fallback_module, "_SMALL_SUBSET_BATCH_QUERY_THRESHOLD", -1
    )
...
async def test_query_with_fallback_can_recurse_into_empty_subset_branch(
    ...
    monkeypatch.setattr(
        status_fallback_module, "_SMALL_SUBSET_BATCH_QUERY_THRESHOLD", 0
    )

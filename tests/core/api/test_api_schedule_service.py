...
async def test_schedule_endpoint_get_uses_mesh_candidates_for_mesh_groups() -> None:
    ...
    assert result == [{"id": 7}]
    endpoint._get_mesh_schedules_by_candidates.assert_awaited_once_with(
        ["03ab0000000000a1"]
    )
...
@pytest.mark.asyncio
async def test_schedule_endpoint_add_validates_matching_time_and_event_lengths() -> (
    None
):
    endpoint = _make_endpoint()
...
async def test_get_mesh_schedules_by_candidates_returns_first_populated_rows() -> None:
    async def execute_candidate_request(
        *, candidate_id: str, **_: object
...
@pytest.mark.asyncio
async def test_add_mesh_schedule_by_candidates_refreshes_full_candidate_set_after_success() -> (
    None
):
    ...
    async def get_rows(
        *, candidate_device_ids: list[str], raise_on_total_failure: bool = True
    ) -> list[ScheduleTimingRow]:
    ...
    async def execute_candidate_request(
        *, candidate_id: str, request, **_: object
    ) -> tuple[bool, object | None, Exception | None]:
    ...
    result = await add_mesh_schedule_by_candidates(
        ...
        encode_mesh_schedule_json=lambda days, times, events: (
            f"{days}:{times}:{events}"
        ),
    )
...
@pytest.mark.asyncio
async def test_delete_mesh_schedules_by_candidates_raises_latest_error_when_all_batches_fail() -> (
    None
):
    async def execute_candidate_request(
        *, candidate_id: str, **_: object

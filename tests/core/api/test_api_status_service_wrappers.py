...
@pytest.mark.asyncio
async def test_rest_facade_query_connect_status_wrapper_preserves_typed_result() -> (
    None
):
...
async def test_rest_endpoint_surface_forwards_typed_batch_metric_callback() -> None:
    ...
    status_endpoints = SimpleNamespace(query_device_status=AsyncMock(return_value=rows))
...
async def test_rest_facade_misc_endpoint_wrappers_preserve_payload_contracts() -> None:
    ...
    assert (
        await rest_facade_endpoint_methods.get_mqtt_config(
            cast("LiproRestFacade", facade)
        )
        == mqtt_payload
    )
    ...
    assert (
        await rest_facade_endpoint_methods.get_city(cast("LiproRestFacade", facade))
        == city_payload
    )
    assert (
        await rest_facade_endpoint_methods.query_user_cloud(
            cast("LiproRestFacade", facade)
        )
        == user_cloud_payload
    )
...
async def test_query_device_status_skips_empty_batch_rows_in_merged_results() -> None:
    async def _request(_path: str, body: JsonObject) -> dict[str, list[dict[str, str]]]:

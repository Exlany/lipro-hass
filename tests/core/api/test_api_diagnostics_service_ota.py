...
async def test_query_ota_info_skips_richer_v2_payload_when_v1_has_valid_rows() -> None:
    ...
    assert [
        args.args
        for args in iot_request.await_args_list
        if args.args[0] == PATH_QUERY_OTA_INFO_V2
    ] == [
        (
            PATH_QUERY_OTA_INFO_V2,
            {"deviceId": "mesh_group_1", "deviceType": "ff000001"},
...
@pytest.mark.asyncio
async def test_query_ota_info_probes_richer_v2_payload_when_v1_v2_rows_invalid() -> (
    None
):
    row = {"deviceType": "ff000001", "latestVersion": "1.0.1"}
    iot_request = AsyncMock(
...
@pytest.mark.asyncio
async def test_query_ota_info_with_outcome_prefers_primary_recovery_over_later_controller_degradation() -> (
    None
):
    row = {"deviceType": "ff000001", "latestVersion": "1.0.1"}
    iot_request = AsyncMock(

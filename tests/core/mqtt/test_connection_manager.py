...
def test_connection_manager_handle_disconnect_clears_state_and_reports_callback_error() -> (
    None
):
...
@pytest.mark.asyncio
async def test_connection_manager_consume_task_exception_returns_none_for_cancelled() -> (
    None
):
...
@pytest.mark.asyncio
async def test_connection_manager_run_connection_loop_retries_after_mqtt_error() -> (
    None
):
    manager = MqttConnectionManager()
    set_last_error = MagicMock()
...
@pytest.mark.asyncio
async def test_connection_manager_run_connection_loop_increases_backoff_after_repeated_failure() -> (
    None
):
    manager = MqttConnectionManager()
    set_last_error = MagicMock()
...
@pytest.mark.asyncio
async def test_connection_manager_run_connection_loop_reports_unexpected_runtime_error(
    caplog,
) -> None:
    manager = MqttConnectionManager()
    set_last_error = MagicMock()
    ...
    assert isinstance(err, RuntimeError)
    assert str(err) == "loop boom"
    handle_disconnect.assert_called_once_with(
        "Unexpected MQTT loop error (RuntimeError)"
    )
    assert sleep_calls == [MQTT_RECONNECT_MIN_DELAY]
...
@pytest.mark.asyncio
async def test_connection_manager_run_connection_loop_stops_on_cancelled_error() -> (
    None
):
    manager = MqttConnectionManager()
    set_last_error = MagicMock()
...
@pytest.mark.asyncio
async def test_connection_manager_run_connection_loop_reraises_programming_error() -> (
    None
):
    manager = MqttConnectionManager()
    set_last_error = MagicMock()
...
@pytest.mark.asyncio
async def test_connection_manager_run_connection_loop_resets_backoff_after_success() -> (
    None
):
    manager = MqttConnectionManager()
    set_last_error = MagicMock()

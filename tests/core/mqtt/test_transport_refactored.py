...
def test_mqtt_package_keeps_transport_out_of_package_exports() -> None:
    mqtt_package_module = importlib.import_module("custom_components.lipro.core.mqtt")
...
def test_transport_initializes_explicit_runtime_owner_contract() -> None:
    ...
    assert (
        transport._runtime_owner.subscription_manager is transport._subscription_manager
    )
    assert (
        transport._runtime_owner.connect_and_listen_entrypoint
        == transport._connect_and_listen
    )
...
def test_transport_error_helpers_delegate_to_connection_manager() -> None:
    ...
    assert transport._invoke_callback(callback, "on_message", "dev1", {}) is False
...
@pytest.mark.asyncio
async def test_transport_finalize_connection_task_delegates_to_connection_manager() -> (
    None
):
    transport = _make_transport()
    connection_manager = MagicMock(spec=MqttConnectionManager)

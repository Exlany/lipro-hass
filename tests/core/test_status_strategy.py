...
def test_normalize_lookup_key_fallbacks_when_normalizer_raises() -> None:
    def _raise_value_error(_: str) -> str:
        raise ValueError("boom")

    assert _normalize_lookup_key("  DeV-01  ", _raise_value_error) == "dev-01"


def test_normalize_lookup_key_fallbacks_when_normalizer_returns_non_string() -> None:
    def _return_non_string(_: str) -> object:
        return 123

    assert _normalize_lookup_key("  DeV-02  ", _return_non_string) == "dev-02"


def test_normalize_lookup_key_fallbacks_when_normalizer_raises_type_error() -> None:
    def _raise_type_error(_: str) -> str:
        raise TypeError("invalid type")

    assert _normalize_lookup_key("  DeV-03  ", _raise_type_error) == "dev-03"

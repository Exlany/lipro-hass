...
def test_coerce_total_count_handles_supported_inputs(total: object, expected: int) -> None:
...
def test_canonical_page_has_more(offset: int, count: int, total: object, expected: bool) -> None:
...
def test_record_snapshot_device_splits_group_outlet_and_gateway(make_device) -> None:
    ...
    record_snapshot_device(
        normalized_row=cast(DeviceRow, {"serial": "group1", "iotDeviceId": "group-iot"}),
        device=group_device,
        assembly=assembly,
    )
    record_snapshot_device(
        normalized_row=cast(DeviceRow, {"serial": "outlet1", "iotDeviceId": "outlet-iot"}),
        device=outlet_device,
        assembly=assembly,
    )
    record_snapshot_device(
        normalized_row=cast(DeviceRow, {"serial": "gateway1", "iotDeviceId": "gateway-iot"}),

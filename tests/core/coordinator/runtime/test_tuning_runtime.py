...
class TestTuningAlgorithm:
    def test_compute_adaptive_batch_size_increase(
        self, tuning_runtime: TuningRuntime
    ) -> None:
        for _ in range(6):
            tuning_runtime.record_batch_metric(
                batch_size=32, duration=1.0, device_count=30
            )
        assert tuning_runtime.compute_adaptive_batch_size() == 40

    def test_compute_adaptive_batch_size_decrease(
        self, tuning_runtime: TuningRuntime
    ) -> None:
        for _ in range(6):
            tuning_runtime.record_batch_metric(
                batch_size=32, duration=4.0, device_count=30
            )
        assert tuning_runtime.compute_adaptive_batch_size() == 24

    def test_compute_adaptive_batch_size_no_change(
        self, tuning_runtime: TuningRuntime
    ) -> None:
        for _ in range(6):
            tuning_runtime.record_batch_metric(
                batch_size=32, duration=2.0, device_count=30
            )
        assert tuning_runtime.compute_adaptive_batch_size() == 32

    def test_compute_adaptive_batch_size_min_bound(
        self, tuning_runtime: TuningRuntime
    ) -> None:
        tuning_runtime.apply_batch_size_adjustment(16)
        for _ in range(6):
            tuning_runtime.record_batch_metric(
                batch_size=16, duration=4.0, device_count=15
            )
        assert tuning_runtime.compute_adaptive_batch_size() == 16

    def test_compute_adaptive_batch_size_max_bound(
        self, tuning_runtime: TuningRuntime
    ) -> None:
        tuning_runtime.apply_batch_size_adjustment(64)
        for _ in range(6):
            tuning_runtime.record_batch_metric(
                batch_size=64, duration=1.0, device_count=60
            )
...
class TestTuningAdjuster:
    ...
    def test_apply_batch_size_adjustment_no_change(
        self, tuning_runtime: TuningRuntime
    ) -> None:

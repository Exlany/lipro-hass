...
class TestStateReader:
    ...
    def test_get_device_by_id(
        self, state_runtime: StateRuntime, mock_device: LiproDevice
    ) -> None:
...
class TestStateUpdater:
    ...
    async def test_apply_properties_update(
        ...
        changed = await state_runtime.apply_properties_update(
            mock_device, properties, source="test"
        )

        assert changed is True
        cast(MagicMock, mock_device.update_properties).assert_called_once_with(
            properties
        )
    ...
    async def test_batch_update_properties(
        ...
        changed_count = await state_runtime.batch_update_properties(
            updates, source="batch"
        )
...
class TestStateIndexManager:
    ...
    def test_unregister_entity_ignores_missing_entity(
    ...
    def test_get_entities_for_device_returns_snapshot(
        self,
        state_runtime: StateRuntime,
        mock_device: LiproDevice,
        mock_entity: MagicMock,
    ) -> None:
        """Returned entity lists should not expose the internal device bucket."""
        entities = state_runtime.get_entities_for_device(mock_device.serial)
    ...
    def test_rebuild_device_index_accepts_explicit_runtime_alias_projection(
        ...
        devices = {
            mock_device.serial: mock_device,
            projected_device.serial: projected_device,
        }

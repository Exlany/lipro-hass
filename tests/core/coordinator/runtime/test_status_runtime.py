...
class TestStatusExecutor:
    ...
    @pytest.mark.parametrize("exception_cls", [RuntimeError, TypeError])
    @pytest.mark.asyncio
    async def test_status_executor_isolates_single_device_apply_failures(
        ...
        async def _apply(device, properties, source):
            ...
            if device.serial == "device1":
                raise exception_cls("apply-boom")

...
def _replace_devices(devices: dict[str, LiproDevice], snapshot_devices: dict[str, LiproDevice]) -> None:
    devices.clear()
...
async def test_polling_service_refresh_devices_updates_state_and_notifies() -> None:
    ...
    service = CoordinatorPollingService(
        ...
        replace_devices=lambda snapshot_devices: _replace_devices(devices, snapshot_devices),
...
@pytest.mark.asyncio
async def test_polling_service_refresh_snapshot_syncs_mqtt_when_transport_exists() -> None:
    devices: dict[str, LiproDevice] = {}

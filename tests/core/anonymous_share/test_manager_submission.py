...
@pytest.mark.asyncio
async def test_aggregate_developer_feedback_uses_primary_scope_client_without_overwriting_default_outcome() -> (
    None
):
    root_manager = AnonymousShareManager()
    default_manager = root_manager.for_scope("__default__")
    ...
    default_submit = AsyncMock(
        return_value=build_operation_outcome(
            kind="failed", reason_code="default_client"
        )
    )
...
def test_aggregate_views_share_submit_outcome_state_without_overwriting_scoped_outcome() -> (
    None
):
    root_manager = AnonymousShareManager()

...
def test_phase71_archived_route_truth_is_distinct_from_latest_v1_20_archive_route() -> (
    None
):
    ...
    assert (
        ...
        in _ARCHIVED_V130_ROADMAP.read_text(encoding="utf-8")
    )


def test_phase75_context_exists_for_latest_archived_closeout_route() -> None:
    context_path = (
        _ROOT / ".planning" / "phases" / _PHASE75_DIR / f"{_PHASE75}-CONTEXT.md"
    )

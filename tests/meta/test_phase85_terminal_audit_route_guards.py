...
def test_phase85_terminal_audit_artifact_covers_required_areas_and_routing_columns() -> (
    None
):
    audit_text = (
    ...
    assert (
        "| `custom_components/lipro/core/anonymous_share/share_client.py` | Medium | closed in Phase 86 |"
        in audit_text
    )
    assert (
        "| `custom_components/lipro/runtime_infra.py` | Medium | closed in Phase 86 |"
        in audit_text
    )
    assert (
        "| `tests/core/api/test_api_diagnostics_service.py` | Medium | route next | `Phase 87`"
        in audit_text
    )


def test_phase85_baseline_refresh_truth_blocks_stale_topology_and_backoff_stories() -> (
    None
):
...
def test_phase85_verification_matrix_and_closeout_truth_register_current_audit_route() -> (
    None
):
    verification_text = (
    ...
    evidence_text = (
        _ROOT / ".planning" / "reviews" / "V1_23_EVIDENCE_INDEX.md"
    ).read_text(encoding="utf-8")
    ...
    assert (
        "## Phase 87 Assurance Hotspot Decomposition / No-Regrowth Guards"
    ...
    assert "V1_23_TERMINAL_AUDIT.md" in evidence_text

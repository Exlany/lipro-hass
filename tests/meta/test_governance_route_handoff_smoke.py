"""Focused current-route smoke guards for the current route handoff."""
...
def test_route_handoff_docs_and_ledgers_stay_in_sync() -> None:
    ...
    assert "## Historical Archived Milestone" in roadmap_text
    expected_plan_line = f"- **Plan:** `{CURRENT_MILESTONE_COMPLETED_PLAN_COUNT} of {CURRENT_MILESTONE_TOTAL_PLAN_COUNT}`"
    expected_percent = (
        (CURRENT_MILESTONE_COMPLETED_PLAN_COUNT * 100)
        // CURRENT_MILESTONE_TOTAL_PLAN_COUNT
        if CURRENT_MILESTONE_TOTAL_PLAN_COUNT
        else 0
    )
    expected_bar = "█" * (expected_percent // 10) + "░" * (
        10 - (expected_percent // 10)
    )
...
def test_gsd_fast_path_matches_current_archived_route_story() -> None:
    ...
    for phase_number in CURRENT_MILESTONE_PENDING_PHASES:
        phase_progress = _as_mapping(phase_by_number[phase_number])
        assert _as_str(phase_progress["status"]) in {
            "pending",
            "not_started",
            "researched",
        }

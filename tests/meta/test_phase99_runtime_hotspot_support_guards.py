...
_ARCHIVED_V127_ROADMAP = _ROOT / ".planning" / "milestones" / "v1.27-ROADMAP.md"
_ARCHIVED_V127_REQUIREMENTS = (
...
_STATUS_FALLBACK = (
    _ROOT / "custom_components" / "lipro" / "core" / "api" / "status_fallback.py"
)
_STATUS_FALLBACK_SUPPORT = (
    _ROOT
    / "custom_components"
    / "lipro"
    / "core"
...
def test_phase99_bundle_stays_visible_as_completed_predecessor() -> None:
    ...
    roadmap_text = _read(_ARCHIVED_V127_ROADMAP)
...
def test_phase99_maps_keep_predecessor_guard_footprint() -> None:
    ...
    assert (
        "tests/meta/test_phase99_runtime_hotspot_support_guards.py" in file_matrix_text
    )
    assert (
    ...
    assert (
        "tests/meta/test_phase99_runtime_hotspot_support_guards.py" in verification_text
    )
    assert (

...
_SCOPE_TEST = (
    _ROOT / "tests" / "core" / "anonymous_share" / "test_manager_scope_views.py"
)
_PHASE_DIR = (
    _ROOT / ".planning" / "phases" / "109-anonymous-share-manager-inward-decomposition"
)
...
def test_phase109_predecessor_bundle_remains_visible_under_phase110_route() -> None:
    ...
    assert (
        "Phase 109 Anonymous-share Manager Inward Decomposition Note" in dev_arch_text
    )
...
def test_phase109_ledgers_testing_and_file_matrix_freeze_predecessor_story() -> None:
    ...
    assert (
        "## Phase 109 Anonymous-share Manager Inward Decomposition" in verification_text
    )
    for token in (
        CURRENT_MILESTONE_DEFAULT_NEXT,
        "tests/meta/test_phase109_anonymous_share_manager_inward_decomposition_guards.py",

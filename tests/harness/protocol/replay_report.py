...
def _build_explicit_family_coverage(
    ...
    for family in EXPLICIT_REPLAY_ASSURANCE_FAMILIES:
        family_scenarios = [
            scenario for scenario in scenarios if scenario["family"] == family
        ]
        coverage.append(
            {
                "family": family,
                ...
                "scenario_ids": [
                    scenario["scenario_id"] for scenario in family_scenarios
                ],
...
def build_replay_scenario_summary(
    ...
    *,
    ...
    recent_command_traces = _as_list(
        telemetry_views.diagnostics["runtime"]["recent_command_traces"]
    )

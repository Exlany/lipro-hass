...
- [ ] `docs/release`: 若修改 `README.md` / `README_zh.md` / `CONTRIBUTING.md` / `SUPPORT.md` / `SECURITY.md` / `.github/*` / release workflow / maintainer docs，已同步 `docs/TROUBLESHOOTING.md`、`docs/MAINTAINER_RELEASE_RUNBOOK.md`、`.planning/baseline/GOVERNANCE_REGISTRY.json`、`.planning/reviews/V1_5_EVIDENCE_INDEX.md` 与相关导航入口，并明确记录任何 contract change（禁止 silent carry-forward）；若涉及 continuity / custody / delegate 叙事，必须同步 `.github/CODEOWNERS` 并明确说明“无隐藏 maintainer / 无 undocumented delegate”，以及是否需要 `freeze new tagged releases and new release promises` 或保持现状；若涉及 release 叙事，还需说明 `provenance` / `SBOM` / `signing` / `code scanning` / firmware metadata 是否本次变更或保持对齐；若本次是 maintainer-only `break-glass verify-only` / `non-publish rehearsal`，也必须在 Summary 中显式说明

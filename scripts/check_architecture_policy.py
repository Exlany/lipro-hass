...
def _validate_policy_inventory(root: Path) -> list[str]:
    ...
    if "## Targeted Regression Bans" not in text:
        errors.append(
            "ARCHITECTURE_POLICY missing '## Targeted Regression Bans' section"
        )
...
def _check_import_rule(root: Path, rule: ArchitecturePolicyRule) -> list[str]:
    governed_paths, missing_governed = resolve_policy_paths(
        rule.governed_targets, root=root
    )
...
def _check_file_contains_rule(root: Path, rule: ArchitecturePolicyRule) -> list[str]:
    if len(rule.governed_targets) != 1:
        return [
            f"{rule.rule_id}: file_contains mode requires exactly one governed file"
        ]
    file_path = root / rule.governed_targets[0]
    if not file_path.exists():
        return [
            f"{rule.rule_id}: governed file missing: {_relative(file_path, root=root)}"
        ]
    text = file_path.read_text(encoding="utf-8")
    missing = [
        signal for signal in rule.allowed_or_required_signals if signal not in text
    ]
    return [
...
def _check_ordered_tokens_rule(root: Path, rule: ArchitecturePolicyRule) -> list[str]:
    if len(rule.governed_targets) != 1:
        return [
            f"{rule.rule_id}: ordered_tokens mode requires exactly one governed file"
        ]
    file_path = root / rule.governed_targets[0]
    if not file_path.exists():
        return [
...
def _check_all_contains_disjoint_rule(
    root: Path, rule: ArchitecturePolicyRule
) -> list[str]:
    file_path = root / rule.governed_targets[0]
...
def _check_property_contains_disjoint_rule(
    root: Path, rule: ArchitecturePolicyRule
) -> list[str]:
    target = rule.governed_targets[0]
    if "::" not in target:
        return [
            f"{rule.rule_id}: property_contains_disjoint requires '<file>::<Class>' target"
        ]
    ...
    if missing:
        errors.append(
            f"{rule.rule_id}: {target} missing required properties: {missing}"
        )
    if forbidden:
        errors.append(
            f"{rule.rule_id}: {target} exposes forbidden properties: {forbidden}"
        )
    return errors


def _check_file_contains_disjoint_rule(
    root: Path, rule: ArchitecturePolicyRule
) -> list[str]:
    file_path = root / rule.governed_targets[0]
    text = file_path.read_text(encoding="utf-8")
...
def _check_top_level_bindings_disjoint_rule(
    root: Path, rule: ArchitecturePolicyRule
) -> list[str]:
    file_path = root / rule.governed_targets[0]
...
def validate_governance_story(root: Path) -> list[str]:
    ...
    for relative_path, required_tokens in _DOC_REQUIRED_TOKENS.items():
        ...
        for token in required_tokens:
            if token not in text:
                errors.append(
                    f"{_relative(root / relative_path, root=root)} missing governance token: {token}"
                )
    ...
    for relative_path, forbidden_tokens in _DOC_FORBIDDEN_TOKENS.items():
        ...
        for token in forbidden_tokens:
            if token in text:
                errors.append(
                    f"{_relative(root / relative_path, root=root)} still contains forbidden legacy token: {token}"
                )
...
def run_checks(root: Path | None = None) -> list[str]:
    ...
    errors: list[str] = []
    if has_policy_text(resolved_root):
...
def main() -> int:
    ...
    parser.add_argument(
        "--check", action="store_true", help="validate architecture policy enforcement"
    )
    parser.add_argument(
        "--report",
        action="store_true",
        help="print architecture policy inventory summary",
    )
    args = parser.parse_args()

    root = policy_root(Path(__file__))
    policy_available = has_policy_text(root)
    structural_rules = load_structural_rules(root) if policy_available else {}
    targeted_bans = load_targeted_bans(root) if policy_available else {}
    ...
    if args.check:
        if not policy_available:
            sys.stdout.write(
                "Skipping governance-backed architecture policy inventory: "

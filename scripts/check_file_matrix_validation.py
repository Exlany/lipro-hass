...
FILE_MATRIX_PATH = Path(".governance/reviews/FILE_MATRIX.md")

PROJECT_PATH = Path(".governance/PROJECT.md")

ROADMAP_PATH = Path(".governance/ROADMAP.md")

STATE_PATH = Path(".governance/STATE.md")

REQUIREMENTS_PATH = Path(".governance/REQUIREMENTS.md")

VERIFICATION_MATRIX_PATH = Path(".governance/baseline/VERIFICATION_MATRIX.md")
...
CODEBASE_MAP_DIR = Path(".governance/codebase")
...
ACTIVE_DOC_PATHS = [
    ...
    Path(".governance/baseline/ARCHITECTURE_POLICY.md"),
    AGENTS_PATH,
    CLAUDE_COMPAT_PATH,
]
...
def has_governance_governance(root: Path) -> bool:
    """Return whether internal governance-backed governance assets are present."""
    governance_root = root / ".governance"
    required_paths = (
        root / FILE_MATRIX_PATH,
        root / VERIFICATION_MATRIX_PATH,
        root / AGENTS_PATH,
    )
    return governance_root.exists() and all(path.exists() for path in required_paths)
...
def _looks_like_repo_path(candidate: str) -> bool:
    ...
    return candidate.endswith(
        (".py", ".md", ".yml", ".yaml", ".json", ".toml", ".sh")
    ) and candidate.startswith(prefixes)
...
def validate_registry_truth_families() -> list[str]:
    ...
    if duplicate_family_keys:
        ...
        errors.append(
            f"check_file_matrix_registry.py has duplicate truth-family keys: {joined}"
        )

    if duplicate_paths:
        joined = ", ".join(sorted(duplicate_paths))
        errors.append(
            f"check_file_matrix_registry.py duplicates override paths across families: {joined}"
        )
    ...
    if tooling_family_paths is None or not required_tooling_paths.issubset(
        tooling_family_paths
    ):
        errors.append(
...
def validate_codebase_map_policy(root: Path) -> list[str]:
    ...
    for token in ("!.governance/codebase/", "!.governance/codebase/*.md"):
...
def run_checks(root: Path) -> list[str]:
    ...
    if not has_governance_governance(root):
        return validate_registry_truth_families()
